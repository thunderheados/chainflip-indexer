...
def get_balance(address: str, height: int) -> dict:
    if indexer.state.chainflip_height < height:
        raise InvalidBlockHeight() 


    block_hash = indexer.safely_execute(indexer.chainflip_nodes, "get_block_hash", params=[height])
    ...
    stakes = Stake.select().where(Stake.address==address, Stake.initiated_height <= height) # initiated_height <= height
