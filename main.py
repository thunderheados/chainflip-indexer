...
class Indexer:
        ...
        for obj in objs:
            ...
            while r > 0:
                try:
                    ...
                    self.logger.warning(
                        "Failed to execute function {} on {}. Error: {}. Retries: {}".format(
                            func, obj, e, r
                        )
                    )
    ...
    def watch_stakes(self):
        while True:
            ...
            for stake in stakes:
                Stake.create(hash = stake["transactionHash"].hex(), amount = stake["args"]["amount"], initiated_height = stake["blockNumber"])

            self.state.ethereum_height = current_height + 1
            self.state.save()

    def get_confirmations(self, block: int):
